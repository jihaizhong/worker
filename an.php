...
use Workerman\Worker; require 'workerman/Autoloader.php'; $clients = []; 
//保存客户端信息 // 创建一个Worker监听9090端口，使用websocket协议通讯 
...
function syncUsers() { 
	...
	foreach($clients as $ip=>$client){ 
		...
		//想要改变作用域外面的数组,就global一下 global $clients; /
		//验证客户端用户名在3-20个字符 
